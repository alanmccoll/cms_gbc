...
class CRM_Reportplus_Form_Contribute_Summary extends CRM_Reportplus_Form {
  ...
  protected $_customGroupExtends = array('Contribution', 'Address', 'Contact', 'Individual', 'Household', 'Organization');
  ...
  public function __construct() {
    ...
    ) + $this->addAddressFields(TRUE, TRUE);
    ...
    $this->_options['calculateMode'] = [
      'type' => 'checkbox',
      'title' => E::ts('Calculate Mode'),
    ];
    $this->_options['calculateMedian'] = [
      'type' => 'checkbox',
      'title' => E::ts('Calculate Median'),
    ];
  }
...
ROUND(AVG({$this->_aliases['civicrm_contribution_soft']}.amount), 2) as civicrm_contribution_soft_soft_amount_avg";
    ...
    if ($this->_params['calculateMedian']) {
  ...
  public function buildRows($sql, &$rows) {
    // FIXME: Commented to solve issue https://lab.civicrm.org/dev/core/issues/1252
